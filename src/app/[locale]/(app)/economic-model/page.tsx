    ...
    <div className={`flex justify-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="relative h-[170px] w-[1200px] overflow-hidden rounded-none md:h-[398px] md:rounded-t-[80px]">
        <div className="size-full">
          <div id="carousel-image" className="size-full bg-[url('/images/architecture_header.png')] bg-contain bg-no-repeat" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className={`flex justify-center transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="mt-[50px] w-[1200px] px-[10px] text-center text-3xl">
        <div id="carousel-text" className="mb-[15px]">{t('title1')}</div>
        <div>{t('title2')}</div>
      </div>
    </div>
  );
...
const AdvantageCard: React.FC<AdvantageCardProps> = ({ index, text }) => {
  return (
    <div className="flex h-[350px] w-full flex-col rounded-none bg-white px-[30px] md:rounded-[25px] hover:cursor-pointer hover:shadow-lg">
...
const Chart: React.FC = () => {
  ...
  return (
    <div className={`flex justify-center pb-[100px] ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div ref={containerRef} className={`h-[350px] w-[1200px] sm:h-[550px] md:h-[900px] ${isVisible ? 'animate-slide-in-bottom' : ''}`}>
