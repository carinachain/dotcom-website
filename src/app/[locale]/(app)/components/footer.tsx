...
const Footer: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-gray-300">
      ...
      <div
        ref={footerRef}
        className={`relative min-h-[400px] w-full overflow-hidden rounded-none bg-slate-800 transition-opacity duration-1000
        xl:w-[1200px] xl:rounded-t-[50px] ${isInView ? 'opacity-100' : 'opacity-0'}
        `}
      >
        <div className={`${isInView ? 'animate-slide-in-bottom' : ''}`}>
          ...
          <div className="p-[50px]">
            <div className="pb-[36px] text-xl text-orange-500">
              {t('intro')}
            </div>
...
const FooterSocialIcon: React.FC<{
  href: string;
  iconSrc: string;
  alt: string;
}> = ({ href, iconSrc, alt }) => (
